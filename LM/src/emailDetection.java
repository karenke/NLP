...
public class emailDetection {
	public static void main(String []args)throws IOException{
		...
		Table beck = new Table("beck-s",beck_bi,beck_un);
		...
		Table farmer = new Table("farmer-d",farmer_bi,farmer_un);
		...
		Table kaminski = new Table("kaminski-v",kaminski_bi,kaminski_un);
		
		String fname = "./data/email/validation.txt";
		
		ArrayList<String> detection = new ArrayList();
		detection.addAll(runValidation(fname,beck,farmer,kaminski));
		
		fname = "./data/email/test.txt";
		detection.addAll(runValidation(fname,beck,farmer,kaminski));
		
		out = "./data/email/emailDetection";
		...
		while(s.hasNext()){
			...
			for(String ss: sentences){
				...
				for(String bigr: setBigram){
					...
					valueb = b.getProbablity(dividBigr[0], dividBigr[1])+valueb;
					valuef = f.getProbablity(dividBigr[0], dividBigr[1])+valuef;
					valuek = k.getProbablity(dividBigr[0], dividBigr[1])+valuek;
				}
			}
